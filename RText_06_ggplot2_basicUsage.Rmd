...
この場合には，データを`filter()`関数(->[参考](./RText_05_DataHandling.html#抽出))を用いて種類ごとに分け，それぞれを描画する．ただし，この場合，`coord_cartesian(xlim =)`関数を使ってx軸の変域を設定しないと，3つのグラフのx軸の変域がバラバラになってしまう．
