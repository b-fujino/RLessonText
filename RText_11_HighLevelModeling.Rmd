...
以下、カテゴリカル変数と連続変数の交互作用、連続変数同士の交互作用、カテゴリカル変数同士の交互作用についてそれぞれ説明する。

## カテゴリカル変数と連続変数の交互作用

:::practice
先ほどの[データ](./practice/example_11_Dammy2.csv)において，学歴と就労年数の交互作用が平均月収にどのような影響を与えるかを調べよ．
:::
...
:::practice
先ほどの[月収データ](./practice/example_11_Dammy2.csv)において，さらに学歴と残業時間の交互作用が平均月収にどのような影響を与えるかを調べよ．
:::

```{r}
data <- read.csv("./practice/example_11_Dammy2.csv")
data$学歴<- as.factor(data$学歴)
fit_interaction <- lm(平均月収 ~ 性別 + 学歴 * (就労期間 + 残業時間), data = data)
summary(fit_interaction)
```
...
:::practice
先ほどの[月収データ](./practice/example_11_Dammy2.csv)において，さらに残業時間と就労期間の交互作用が平均月収にどのような影響を与えるかを調べよ．
:::
...
回帰分析での交互作用においては，**ある説明変数の目的変数に対する影響力が，他の説明変数によって変わる**という解釈がなされる．
