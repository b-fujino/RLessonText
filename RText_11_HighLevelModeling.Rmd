...
つまり，性別が1，すなわち男性の場合には平均月収が約8000円高くなる，就労期間が1年長くなると8410円高くなる，残業時間が1時間伸びると2820円高くなる，ということが分かる．
...
ダミー変数を導入してカテゴリカル変数を回帰分析の中で検討することのメリットとして，カテゴリカル変数を統制変数とすることができる点があるが，逆に連続変数を統制変数としてカテゴリカル変数の各水準間の**より真なる差**を評価するために用いることができる点もあげられる．

例えば，先の月給の例の場合，単純な性別だけで比較すると以下の通りとなる．
```{r}
fit<-lm(平均月収 ~ 性別, data = data)
summary(fit)
```

この結果では，男性の場合，平均月収が約15万円も差があるということになる．
単純にこの会社の男女で比較した場合，女性は非常に低い額に抑えられている，ということになる．実際に単純に男女で平均月収を比較したものを図示すると数のようになる．

```{r}
ggplot(data, aes(x = 性別, y = 平均月収,color=性別)) +geom_jitter(width = 0.2)
...
回帰分析での交互作用においては，**ある説明変数の目的変数に対する影響力が，他の説明変数によって変わる（調整される）**という解釈がなされる．
...
上がるということが分かる．すなわち，高卒と大卒でこの会社では昇給のスピードは同じになっているということである．

一方，学歴2と就労期間の交互作用は有意である．つまり，院卒者は就労期間が長くなると平均月収がどの程度上がるかは高卒者とは有意に異なるということである．具体的には，院卒者は就労期間が1年長くなるごとに平均月収が
...
この結果から，就労期間と残業時間の交互作用は有意であることが分かる．つまり，就労期間が1年長くなるごとに残業代の単価（残業時間が1時間伸びると平均月収がどれくらい上がるか）は約64円ずつ上がっていくことがわかる．
...
これと同様のことを行うのが単純傾斜分析（Simple Slope Analysis）である．回帰分析において連続変数同士の交互作用が有意であった場合に，一方の説明変数を$\pm 1$標準偏差だけ変化させた場合の他方の説明変数の回帰係数が有意になるかどうかを検討する．
...
の人の残業代の単価は約1600円であり，これらはいずれも就労期間が
$平均=`r round(mean(data$就労期間),0)`$年の場合の残業代の単価である約2300円と比べて有意に異なっているることがわかる．
...
方法としては，標準偏差を動かしたい変数（今回であれば就労期間）に対して，まずは平均を引いたうえで（この処理を中心化と呼ぶ），1標準偏差を足す，もしくは1標準偏差を引く処理を行ったデータを用意して，それぞれに同じモデル式`lm()`に与えればよいだけである．
注意すべき点としては+1標準偏差するというのは，データを+方向に動かすのではなく，軸を+方向に動かすことを意味している．つまり，これは―1標準偏差の人たちのデータとなる．
逆に，+1標準偏差するというのは，軸を-方向に動かすことを意味しているので+1標準偏差の人たちのデータとなる．このように処理として行った＋，-と，評価対象として見ている人達がどういう人たちなのかというものが入れ替わることに注意してほしい．
```{r }
#Sを1標準偏差足す
data_p <-data
...
なお，これはあくまで学歴，性別は0の場合，すなわち高卒の女性のみのケースをグラフ化したものである．性別が男性の場合，切片が$`r round(coef(fit_c)[1],2)`$だけ上がり（ただし，この差は有意ではない），学歴が大卒の場合は傾きが$`r round(coef(fit_c)[3],2)`$だけ，院卒の場合は傾きが$`r round(coef(fit_c)[4],2)`$だけ上がる．
