...
<<<<<<< HEAD
# 独立性の検定
独立性の検定とは、例えば以下の表のような「クロス集計表」が作られている時に、列方向に並べた属性変数Aと行方向に並べた属性変数Bが互いに独立しているのか、何等かの関係性があるのかを検定する方法である。**帰無仮説は「互いに独立している」**であり、それが否定することによって「関係性がある」、さらに言えば「$B:\bar{B}$の比が$A$と$\bar{A}$の間で有意に違っている」と結論づける統計方法である。
=======
:::
...
<<<<<<< HEAD
独立性の検定では一般にカイ2乗($\chi^2$)検定というものを用いる。ただし、カイ2乗検定はクロス集計した際のいずれかのセルが5未満の値の場合には正確な値は返してこない場合がある。そこでそうしたケースでも対応できるような方法としてFisherの正確確率検定というものがある。以下では、カイ2乗検定とFisherの正確確率検定の両方の方法について説明する。
=======
比率の検定では一般にカイ2乗($\chi^2$)検定というものを用いる．ただし，カイ2乗検定はクロス集計した際の**いずれかのセルが5未満の値の場合には正確な値は返してこない場合がある**．そこでそうしたケースでも対応できるような方法としてFisherの正確確率検定というものがある．以下では，カイ2乗検定とFisherの正確確率検定の両方の方法について説明する．
>>>>>>> 9b7ac0bd0970f87f315aa1a7cd3934d8df86290e
...
# 比率の差の検定
独立性の検定と類似したものとして「比率の差の検定」というものもある．独立性の検定が要因Aと要因Bのクロス集計表から，**AとBの間に関連性があるかどうか**を検証するのに対して，比率の差の検定は，要因Aの2つの水準（例えば，男性と女性）の間で要因Bの成功率（例えば，合格率）の差が有意であるかどうかを検証するものである．この場合にはZ検定を用いる
...
mx<-sum(mydata_chisq$性別==0 & mydata_chisq$合否==1)
# 他のも以下の書き方をしてもよい．
# d_m_s<- filter(mydata_chisq, 性別==0 & 合否==1) 
# mx<- nrow(d_m_s)

#男性0の総数
mn<-sum(mydata_chisq$性別==0)
# d_m<- filter(mydata_chisq, 性別==0)
# mn<- nrow(d_m)


#女性1の合格1の数
fx<-sum(mydata_chisq$性別==1 & mydata_chisq$合否==1)
#女性1の総数
fn<-sum(mydata_chisq$性別==1)
```

以上のようにして男女別の合格者数がmx, fx, 受験者数がmn, fnとして求まったので，これらを使ってprop.test()関数を実行する．

```{r}
# それぞれの成功数と試行数をベクトルにする わかりやすさのため名前付きベクトルにしておく
success <- c(男性合格者=mx, 女性合格者=fx) 
total <- c(男性受験者=mn, 女性受験者=fn)
# 確認のためそれぞれの合格率を算出
print(success/total)
...
ただ，$3\times 2$以上のクロス集計となる場合には，独立性の検定においてはは3つの水準すべてを考慮したうえで，要因Aと要因Bの独立性（関連性）を検定することになる一方，比率の差の検定の場合は，3つの水準のどれとどれを比較するのか，という話や以下で述べる多重比較の話となり，まったく議論の方向性が異なってくる．
...
## 比率の多重比較
比率の多重比較を行う場合にも，基本的な考え方は平均や分散の多重比較と同様である．1対1の組み合わせを作り，それらのP値を補正して評価する，というものである．比率の多重比較に関しては，分散の多重比較の時と同じく，個別の組み合わせで比較を行った後，p.adjust()関数を用いて補正を行うことで多重比較を行うことができるが，カイ2乗検定を行う場合には`pairwise.prop.test()`関数を用いる．

`pairwise.prop.test()`では，以下の引数をとる．

- 第1引数x = : 比較対象のデータの成功数のベクトル
- 第2引数n = : 比較対象のデータの試行数のベクトル
- p.adjust.method = : 補正方法
...
set.seed(42)
num_samples <- 1000
工程A <- sample(c("良", "不良"), num_samples, replace=T, prob=c(1-0.05, 0.05))
工程B <- sample(c("良", "不良"), num_samples, replace=T, prob=c(1-0.1, 0.1))
工程C <- sample(c("良", "不良"), num_samples, replace=T, prob=c(1-0.12, 0.12))
...
#良品数と総数を計算
不良品 <- c( nrow(filter(mydata_multcomp_prop, 工程A=="不良")), 
          nrow(filter(mydata_multcomp_prop,工程B=="不良")), 
          nrow(filter(mydata_multcomp_prop, 工程C=="不良"))
          )

総数 <- c(nrow(mydata_multcomp_prop), nrow(mydata_multcomp_prop), nrow(mydata_multcomp_prop))


#多重比較
pairwise.prop.test(不良品, 総数, p.adjust.method = "holm")
