...
そこで，ここではafexパッケージのaov_ez関数を使って二要因分散分析を行う方法を紹介する．
...
受講後 <- round(rnorm(num_samples, mean = 60, sd = 10),0)
...
このようなデータの場合，一方理解度テストの「テスト時期」（受講前, 受講後）のみ対応ありであり，「講義方法」（従来型，新型）は対応なしである．このような場合の二要因分散分析は，一方が対応ありである場合の二要因分散分析として扱うことができる．

このように一方のみが対応ありデータの場合，対応ありの要因のことを「被験者内要因」あるいは「Within要因」とよび，対応なしの要因のことを「被験者間要因」あるいは「Between要因」とよぶ．

このようなデータの場合にはaov_ez関数を使うさいに，**between引数の設定をする必要がある**．
