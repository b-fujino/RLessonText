...
一方、説明変数が2つの場合には「二要因分散分析」、3つの場合には「3要因分散分析」と呼ぶ。理論上は説明変数をいくつでも用意することはできるが、説明変数が増えるにつれて解析が複雑になるため、一般的には二要因分散分析程度でとどまることが行われることが多い。
...
次に、[対応ありデータ](#t検定の方法（ワイド形式データ）)での一要因分散分析の例を考えよう。
