...
すなわち，分散分析や回帰分析のいずれの場合も目的変数は間隔尺度か比例尺度となるが，説明変数が**名義尺度，順序尺度**である場合には**分散分析**を，**間隔尺度，比例尺度**である場合には**回帰分析**を用いることとなる．
...
## くり返しの有無
Excelの二元配置分散分析は二元配置分散分析（くり返しなし）と二元配置分散分析（くり返しあり）に分かれる．「くり返しのある」2元配置分散分析とは，2つの要因を組み合わせた条件の下で**複数回の測定**を行っているケースに対応した分析手法である．それぞれの組み合わせで複数回の測定を行っている場合，それぞれの要因の**交互作用（相乗効果・組み合わせ効果）**を検証することができる．交互作用とは，2つの要因が組み合わさった時に初めて現れる効果のことである．
...
## データのパターン
どういうデータの時にどういうデータのパターンになるかを整理すると，以下のようになる．

<div class="WideFigure">
![各分散分析のデータのパターン](./Img/2025_01_06_AnovaDataPattern.png)
</div>
「対応なし」の場合には，必ずしも各水準でデータ数が一致していなくてもよいが，「対応あり」の場合はデータ数は各水準で一致している必要がある．

また，「繰り返しなし」の場合のデータは，各水準で1個のみであるが，「繰り返しあり」の場合は各水準で複数個のデータが存在している必要がある．


なお，上の図ではわかりやすさのためワイド形式でのデータを示しているが，実際にはロング形式でのデータを用いる．
すなわち以下のようなデータ形式にしておく必要がある．


<div class="WideFigure">
![各分散分析のデータのパターン](./Img/2025_01_06_AnovaDataPattern_long.png)
</div>
...
# 準備
分散分析を行うためには，carパッケージを読み込む必要がある．またこれらの読み込みにあたっては，予めcarというパッケージをインストールしておく必要がある．インストールはこれまで何度も出てきているが，Tools->install.packagesから行う．インストールが済んだら，以下の通りに`library()`関数にて，両パッケージを読み込む．

なお，以下では今までと同様に，tidyverseパッケージも読み込んでいる．
...
# 一要因分散分析（対応なし）／被験者間計画一要因分散分析／一元配置分散分析
...
分散分析を実施するコマンドは`aov()`関数である．この関数の引数は以下の通りであり，これは回帰分析の`lm()`と全く同じである．こうした点からも回帰分析と分散分析が親戚関係にあることがうかがえる．

- 第1引数: `目的変数~説明変数`(`~`はチルダ)という形で表現される式を与える
- 第2引数: 目的変数と説明変数を含んだデータフレームを与える．


結果は一旦任意の名前を付けたオブジェクトとして保存し，`summary()`関数で結果を表示する．
...
:::ref
<details>
  <summary>平均平方と平方和の意味</summary>
実はこの平均平方の値($合計の平方和/合計の自由度$)は母分散の不偏推定量そのものである(->[分散の点推定](./RText_04_DescriptiveStatistics.html#分散の点推定))．このことから，要するに分散分析とはデータ全体での分散（平方和）を説明変数の平方和と残差の平方和に**分割**し，残差と説明変数の分散比が有意に異なっているかをF検定によって求めたものといえる．もし，分散比が有意に異なっているならば，説明変数を考慮することは意味があるが，もし分散比に有意な違いがないならば，説明変数を考えることに大して意味はない，ということとなる．
</details>
:::
...
Control <- data.frame(
  condition = "統制",
  score = c(round(truncnorm::rtruncnorm(n=50, mean=6, sd=1, a=0, b=10),1),rep(NA, 5))
)

Lecure <- data.frame(
  condition="講義",   
  score =c(round(truncnorm::rtruncnorm(n=47, mean=6, sd=1, a=0, b=10),1),rep(NA, 8))
)

Discussion <- data.frame(
  condition="議論",
  score =round(truncnorm::rtruncnorm(n=55, mean=8, sd=1, a=0, b=10),1)
)
...
write_csv(mydata, "./practice/08_work_anova.csv")
...
# 一要因分散分析（対応あり）／被験者内計画一要因分散分析／繰り返しのない2元配置分散分析
...
すなわち，「おいしさ」の得点には，「パンの種類」だけでなく，個々のサンプルの得点のつけ方も影響する可能性が考えられる．そこで，「おいしさ」に対するモデルとして，サンプルを識別する変数（今回の例では「被験者No.」）も説明変数に加えたモデルとする必要がある．

すなわち，モデル図としては以下の通りとなる．

![この例題のモデル図](./Img/2024_5_30_OneFactorAnovaWithin.png)

このような説明変数を持った分散分析では，**`aov()`関数を実施する際に`Error()`関数を用いてサンプルを識別する変数を指示する必要がある．
Error()関数は，`Error(サンプル識別変数/説明変数)`という形で指定する．これは説明変数に対してサンプル識別変数を設定する，ということであり，より具体的には`サンプル識別変数:説明変数`という交互作用項を誤差項に含める，ということである．もし説明変数の各水準で各サンプルの測定が1回しか行われていない場合には，このような交互作用は考えることができないため，`Error(サンプル識別変数)`とだけ記述するのでもよい．
...
#分散分析の実施
anova_res_within <- aov(おいしさ~パンの種類 + Error(被験者No./パンの種類), mydata_anova_within_long)
...
#afexを使う
library(afex)
res <-aov_ez(id="被験者No.", dv="おいしさ", within="パンの種類" , data=mydata_anova_within_long, type=3)
summary(res)
```
...
:::ref
<details>
<summary>分散分析とt検定の関係</summary>
対応あるデータでの一要因分散分析は，t検定における対応あるデータでの分析の拡張であり，水準が2つであればt検定を，水準が3つ以上であれば分散分析を行うことになる．
...
:::ref
<details>
<summary>Excelでの一要因分散分析（対応あり）の実施</summary>
実は非常にややこしいのだが，対応ありデータでの一要因分散分析は，**交互作用のない**二要因分散分析とも言うことができる．したがってExcelの分析ツールで対応ありデータでの一要因分散分析を実施する場合には，**2元配置分散分析（繰り返しなし）**を選択する．
...
試作品 <- round(truncnorm::rtruncnorm(n=30, mean=6.3, sd=2, a=1, b=10),0)
従来品 <- round(truncnorm::rtruncnorm(n=30, mean=6.5, sd=2, a=1, b=10),0)
mydata_anova_2way <- data.frame(
  被験者No. = c(1:30),
  試作品 = 試作品,
  従来品 = 従来品
)

write_csv(mydata_anova_2way, "./practice/08_work_anova_2way.csv")
...
# 二要因分散分析(2要因とも対応なし)／被験者間計画二要因分散分析
...
しかしながら，このモデル図は誤りとなる．二要因分散分析では基本的に**交互作用**というものを考慮する必要がある．これは「説明変数同士の掛け算の効果」あるいは「相乗効果」のことである．例えば，品種AはBよりも病気に強い，肥料Xは肥料Yに対して基本的に効果が高い，ということであれば，A＋Xが最も収穫量が高く，B＋Yがもっとも収穫量が低いということになるだろう．しかしながら，既製品同士のBとYは非常に相性が良く，この二つを組み合わせると，AとYを組み合わせたり，BとXを組み合わせたりするよりも収穫量が高くなる，ということもあり得る．こうした**組み合わせたことによって相乗的に生じてくる効果**のことを**交互作用**と呼ぶ．
二要因分散分析では基本的にこうした交互作用も含めたモデル図にする必要があるので，正しくは以下のようなモデル図となる．

![この例題の正しいモデル図](./Img/2024_6_6_2wayAnovaWithInteraction.png)

こうした交互作用を含んだ二要因分散分析を行う方法では，使うコマンドは`aov()`関数と`car`パッケージの`Anova()`関数を用いる．

`Anova()`関数の引数は以下の通りである．

- 第1引数: aov()関数の出力結果を収めたオブジェクト
- 第2引数 type: `type=3`とする(ほかにtype=1と2がある．`type=2`は交互作用を考慮しない場合に用いる．`type=1`はaovと同じ結果となるだけなので用いることはほぼない．）

なお，`car`パッケージは[こちら](#準備)で既に読み込みをしているので，改めてlibrary()関数で読み込む必要はない．



:::ref
<details>
  <summary>分散分析のタイプの違い</summary>
分散分析における「タイプ」の違いについては以下の通りである．

**タイプ1**:
変数をモデルに投入する毎に，その時点での残差とその変数の平方和の比をF検定する．
変数の投入順序によって残差の平方和が変わり得るので，同じモデルでも変数のモデルへの投入順序によって結果が変わることがある．このため，**対応のないデータでの一要因分散分析でのみ使用される**のが一般的である．
...
# 二要因分散分析(2要因とも対応あり)／被験者内計画二要因分散分析

対応ありデータでの2要因分散分析では，さらに**2要因とも対応ありデータ**か**一方だけが対応ありデータ**かによって分析方法が異なる．

まずは，二要因とも対応ありのデータでの分析を説明する．
...
この事例の場合，説明変数として「食パン（水準：A，B，C）」と「苺ジャム（水準：X，Y）」という2つの変数があり，目的変数として「おいしさ（1～10の連続変数）」を設定している．そしてさらに，一人の被験者は3つの食パンすべてに点を付けているし，同じく2つのジャムすべてに点を付けている．言い換えれば，3×2=6通りの組み合わせすべてを試食している．この事例の場合のモデル図は以下のようなものとなる．


![この例題のモデル図](./Img/2024_7_3_2wayAnovaWithInteractionAndPaired.png)


このような対応ありデータ(反復測定のデータ）での二要因分散分析を行う方法はこれまで紹介したようなaov関数をつかう方法もあるが，それらは設定が少々むずかしかったり，データによっては推定がうまく行かないケースがある．
そこで，ここでは**`afex`パッケージ**のaov_ez関数を使って二要因分散分析を行う方法を紹介する．

aov_ez関数の引数は以下の通りである．

- id: 被験者を識別する列
- dv: 従属変数(depend variableの略)の列名
- within: 被験者内要因. 複数ある場合にはc("要因1", "要因2")のように指定する
- between: 被験者間要因. もし被験者間要因がある場合には指定する．今回は省略となる．
- data: 分析に掛けるデータフレーム
- type: タイプIII分散分析を行うかどうかの指定. 1, 2, 3のいずれかを指定する




<!-- 基本的に[一要因分散分析（対応あり）](#一要因分散分析（対応あり）)と大して変わらない．すなわち，使うコマンドは`aov()`関数と`car`パッケージの`Anova()`関数である．違う点は，`aov()`関数に与えるモデル式に交互作用項が加わることと，`Anova()`関数の`type`引数に`3`を指定することである． -->


この例題の場合は以下のようなコードとなる．なお，今回のデータでは，「パンの種類」「苺ジャム」「被験者No.」という3つの変数が存在しているため，ワイド形式では1枚の表で記述するのは難しくなる．このため，今回読み込むデータでも予めロング形式のデータとしている．
<!-- また交互作用項の記述に関しても`*`を用いて省略した記述法を行っている． -->
...
#
mydata_anova_2way_within$個人No. <- as.factor(mydata_anova_2way_within$個人No.)

#分散分析の実施
anova_res_2way_within <- aov(評価~メーカー*ジャム + 個人No., 
                             mydata_anova_2way_within)
...
anova_res_2way_within<- lmerTest::lmer(評価~メーカー*ジャム+(1+メーカー+ジャム||個人No.) , mydata_anova_2way_within)
#残念ながらisSilgularがTRUEになり，パラメータが過剰となってしまった．
Anova(anova_res_2way_within, type=3)
```
...
anova_res_2way_within <- aov_ez(
  id = "個人No.",       # 被験者を識別する列
  dv = "評価",          # 従属変数
  within = c("ジャム", "メーカー"),  # 被験者内要因
  data = mydata_anova_2way_within,
  type = 3              # タイプIII分散分析
)
summary(anova_res_2way_within)
```
...
#個人No.を要因化
mydata_anova_2way_within$個人No. <- as.factor(mydata_anova_2way_within$個人No.)
anova_res_2way_within<-aov(評価~ジャム*メーカー + Error(個人No./(ジャム*メーカー)), mydata_anova_2way_within)
summary(anova_res_2way_within)
...
### 各群の平均と標準偏差の算出
分析に先立って，それぞれの組み合わせの下での平均と標準偏差を算出しておく．
...
# 二要因分散分析(1要因だけ対応あり)／混合計画二要因分散分析

二つある要因のうち，一方の要因については対応ありだが，他方の要因では対応なしの場合もある．
次のような事例を考えよう．

:::practice
ほぼ同レベルの能力をもった大学生100人を集め，50人ずつの2グループに分けて，地域課題に関する講義を実施した．一方の授業は従来から行われている方法での講義（従来型）であり，もう一方は，その先生のオリジナルな新しい方法を導入した講義（新型）である．この講義の受講前と受講後に学生の地域課題に対する理解度を測るテストを実施している．得られたデータは[こちら:](./practice/example_anova_2way_wb.csv)である．このデータを用いて，新型講義の効果を検討せよ．
```{r include=FALSE}
set.seed(32)
num_samples <- 100
受講前 <- round(rnorm(num_samples, mean = 50, sd = 10),0)
受講後 <- round(rnorm(num_samples, mean = 60, sd = 10),0)+c(rep(0, num_samples/2), rep(10, num_samples/2))
group <- rep(c("従来型", "新型"), each = num_samples/2)

data <- tibble(
  個人No.=rep(1:num_samples,2),
  講義方法=rep(group,2),
  テスト時期 = c(rep("受講前",each = num_samples), rep("受講後",each = num_samples)),
  理解度 = c(受講前, 受講後)
)

print(data)

write_csv(data, "./practice/example_anova_2way_wb.csv")
```
:::

このようなデータの場合，理解度テストの「テスト時期」（受講前, 受講後）のみ対応あり（どの学生も受講前・受講後のテストは必ず両方受けている）であり，「講義方法」（従来型，新型）は対応なし（従来型を受講した学生と，新型を受講した学生は異なっており，両方受講した学生はいない）である．
このような場合の二要因分散分析は，一方が対応ありである場合の二要因分散分析となる．

モデル図は以下の通りになる．

![この例題のモデル図](./Img/2024_7_3_2wayAnovaWandBInteractionAndPaired.png)

このように一方のみが対応ありデータの場合，対応ありの要因のことを「被験者内要因」あるいは「Within要因」とよび，対応なしの要因のことを「被験者間要因」あるいは「Between要因」とよぶ．さらに，このようにWithin要因とBetween要因が混在しているようなケースを「混合計画」もしくは「混合モデル」と呼ぶ．


このようなデータの場合にはaov_ez関数を使う際に，**between引数の設定をする必要がある**．

```{r}
library(afex)
mydata_anova_2way_wb <- read.csv(
  "./practice/example_anova_2way_wb.csv",
  header=T,
  fileEncoding="UTF8"
) # データ読み込み

#個人No.を要因化
mydata_anova_2way_wb$個人No. <- as.factor(mydata_anova_2way_wb$個人No.)

anova_res_2way_wb <- aov_ez(
  id = "個人No.",       # 被験者を識別する列
  dv = "理解度",          # 従属変数
  within = "テスト時期",  # 被験者内要因
  between = "講義方法",  # 被験者間要因
  data = mydata_anova_2way_wb,
  type = 3              # タイプIII分散分析
)
summary(anova_res_2way_wb)
```

```{R include=FALSE}
anova_res_2way_wb <- aov(理解度~講義方法*テスト時期 + Error(個人No./(テスト時期)), mydata_anova_2way_wb)
summary(anova_res_2way_wb)

```

以上の結果から，講義方法の主効果，テスト時期の主効果，およびその交互作用が理解度に対して有意な影響を与えていることが示された．


## 下位検定（単純主効果検定）

今回の結果では，講義方法，テスト時期それぞれが有意であったが，この主効果が有意である点は大した意味はない．

すなわち，講義方法の主効果が有意というのは，テストの実施時期を完全に無視して，講義方法だけでデータを分けて従来型と新型の平均点を算出し，両者に差があると言えるのか（水準が違うと平均が違うのか）を見ていることになるが，受講前と受講後をまとめた平均を算出することには何の意味をないのは容易に理解できるだろう．

同様に，テスト時期の主効果が有意というのは，講義方法を完全に無視して，受講前と受講後の平均点を算出し，両者に差があると言えるのか（時期が違うと平均が違うのか）を見ていることになる．当然ながら，従来型と新型をまとめて受講前の平均点を出したり，受講後の平均点を出したりすることに何の意味もない．

したがってあくまで注目するべきは交互作用の結果である．
今回の設定の場合，想定しているのは以下のような結果が得られることである．

- 講義受講前では，従来型群と新型群の間に有意な差はない．
- 講義受講後では，従来型群と新型群の間に有意な差がある（新型の方が成績が良くなる）．

そこで，こういうことを明らかにするための単純主効果検定を実施する必要がある．


### 各群の平均と標準偏差の算出
まずは「従来型-受講前」「従来型-受講後」「新型-受講前」「新型-受講後」のそれぞれの平均と標準偏差を算出しておく．

```{r}
#従来型のデータきりだし
data_old <- filter(mydata_anova_2way_wb, 講義方法 == "従来型")
##従来型の受講前の理解度の平均・標準偏差
mean_old_before <- mean(filter(data_old, テスト時期 == "受講前")$理解度)
sd_old_before <- sd(filter(data_old, テスト時期 == "受講前")$理解度)
##従来型の受講後の理解度の平均・標準偏差
mean_old_after <- mean(filter(data_old, テスト時期 == "受講後")$理解度)
sd_old_after <- sd(filter(data_old, テスト時期 == "受講後")$理解度)

#新型の場合
data_new <- filter(mydata_anova_2way_wb, 講義方法 == "新型")
mean_new_before <- mean(filter(data_new, テスト時期 == "受講前")$理解度)
sd_new_before <- sd(filter(data_new, テスト時期 == "受講前")$理解度)
mean_new_after <- mean(filter(data_new, テスト時期 == "受講後")$理解度)
sd_new_after <- sd(filter(data_new, テスト時期 == "受講後")$理解度)

#データフレーム化
mean_data <- data.frame(
  講義方法 = as.factor(c("従来型", "従来型", "新型", "新型")),
  テスト時期 = as.factor(c("受講前", "受講後", "受講前", "受講後")),
  理解度 = c(mean_old_before, mean_old_after, mean_new_before, mean_new_after),
  sd = c(sd_old_before, sd_old_after, sd_new_before, sd_new_after)
)
...
講義方式を基軸として見る場合，従来型，新型ごとの受講前，受講後の差を見ることになる．今回だと受講前，受講後という2つの群の比較なので，単純にT検定（対応あり）をすればよい．

```{r}
# 従来型の場合
data_old_before <- filter(data_old, テスト時期 == "受講前")
data_old_after <- filter(data_old, テスト時期 == "受講後")
t.test(data_old_before$理解度, data_old_after$理解度, paired=TRUE)

# 新型の場合
data_new_before <- filter(data_new, テスト時期 == "受講前")
data_new_after <- filter(data_new, テスト時期 == "受講後")
t.test(data_new_before$理解度, data_new_after$理解度, paired=TRUE)
```
この結果から，従来型も新型も受講前後では有意に理解度が向上することが確認できた．

### テスト時期を基軸とした単純主効果検定
今度はテスト時期を基軸として見る．この場合，受講前，受講後ごとの従来型，新型の差を見ることになる．この場合も従来型，新型という2つの群の比較なので，単純にT検定をすればよい．ただし注意すべき点として，従来型と新型は異なる学生が受講しているため，対応なしのT検定となるので`paired=FALSE`とする必要がある．．

```{r}
# 受講前の場合
t.test(data_old_before$理解度, data_new_before$理解度, paired=FALSE)

# 受講後の場合
t.test(data_old_after$理解度, data_new_after$理解度, paired=FALSE)
```

この結果から，受講前では従来型と新型の間に有意な差はないが，受講後では新型の方が従来型に比べて有意に理解度が高いことが示された．

### ggplotによる可視化
ggplotでこの結果を可視化すると以下の通りとなる．
`geom_point()`や`geom_line()`は，`position`引数に`position_dodge()`を与えて横にずらして表示させている．

```{r}
# 順番を整理するためにfactor関数を使って再整理する
mean_data$テスト時期 <- factor(mean_data$テスト時期, levels = c("受講前", "受講後"))

# ggplotで棒グラフで表示
g<-ggplot(mean_data, aes(x=テスト時期, y=理解度, fill=講義方法, group=講義方法, color=講義方法)) +
  geom_point(position=position_dodge(width=0.2), size=3)+
  geom_line(position=position_dodge(width=0.2))+
  geom_errorbar(aes(ymin=理解度-sd, ymax=理解度+sd), width=.1, position=position_dodge(width=0.2)) +
  coord_cartesian(ylim = c(35, max(mean_data$理解度) + 20))

plot(g)

```

さらに，差が有意であることを示すブラケットを追加する．
```{r}
# 従来型に対するブラケットを記述
g <- g + 
  annotate("segment",x=0.95, xend=0.95, y=61, yend=75)+
  annotate("segment",x=0.95, xend=1.95, y=75, yend=75)+
  annotate("segment",x=1.95, xend=1.95, y=75, yend=71)+
  annotate("text", x=(0.95+1.95)/2, y=78, label="p<.001,***")

# 新型に対するブラケットを記述
g <- g + 
  annotate("segment",x=1.05, xend=1.05, y=59, yend=87)+
  annotate("segment",x=1.05, xend=2.05, y=87, yend=87)+
  annotate("segment",x=2.05, xend=2.05, y=87, yend=81)+
  annotate("text", x=(1.05+2.05)/2, y=88, label="p<.001,***")


# テスト実施時期での比較
g <- g+
  annotate("segment",x=2.10, xend=2.20, y=mean_data$理解度[4], yend=mean_data$理解度[4])+
  annotate("segment",x=2.20, xend=2.20, y=mean_data$理解度[4], yend=mean_data$理解度[2])+
  annotate("segment",x=2.20, xend=2.10, y=mean_data$理解度[2], yend=mean_data$理解度[2])+
  annotate("text", x=2.4, y=(mean_data$理解度[2]+ mean_data$理解度[4])/2, label="p<.001,***")

plot(g)
```

このように可視化すると，受講前の状態では講義の方式による差はみられなかったが，受講後にはいずれの群も有意に理解度が向上していること，さらには新型の場合には講義後の理解度が従来型に比べて有意に高いことが一目瞭然で把握できる．

:::ref
上記のブラケットまでggplotで表示させようとすると，正直結構面倒くさかった（苦笑）．

ggplotで描画させるのはあくまでグラフまでにして，ブラケットの追加等の装飾は，ggplotの出力結果をpngで保存して，パワーポイント等に貼り付けて行う方が楽かもしれない．
:::
...
:::ref
棚の高さを変えないグループを設けたのは，季節要因など実験で設定した要因以外の要因も売り上げに影響し得るためである．
このような比較のために設けた群のことを「対照群」と呼ぶ．対照群の設定は，実験の信頼性を高めるために非常に重要である．
:::
...
sales_before <- round(rnorm(num_Area_A + num_Area_B, mean = 100, sd = 10),0)
sales_after <- round(rnorm(num_Area_A + num_Area_B, mean = 102, sd = 10),0) + c(rep(11, num_Area_A), rep(0, num_Area_B)
)

data <- tibble(
  店舗No.=rep(1:(num_Area_A + num_Area_B),2),
  条件 = c(rep("実験群", num_Area_A), rep("対照群", num_Area_B), rep("実験群", num_Area_A), rep("対照群", num_Area_B)),
  時期 = c(rep("変更前", each = num_Area_A + num_Area_B), rep("変更後", each = num_Area_A + num_Area_B)),
  売上げ = c(sales_before, sales_after)
)

write_csv(data, "./practice/08_work_anova_3.csv")
...
:::ref
被験者内計画での実験を行う際には，各被験者は設定したすべての実験条件を体験することになる．その場合，どの順序で各条件を体験したのかが結果に影響を与えてしまう可能性がある（例えば，どういう条件であっても最初に体験した条件は印象が強くなる，など）．こうした影響のことを**順序効果**と呼ぶ．被験者内計画での実験では順序効果をいかに除くかが非常に重要である．順序効果を排除する方法として，今回の事例のように，実施順序を入れ替えた群を設け，元の群のデータと実施順序を入れ替えた群のデータを合わせることによって，データ全体として順序効果を相殺する，という方法が一般によく取られる．
...
num_students <- 20
before <- round(truncnorm::rtruncnorm(n=num_students*2, a=1, b=7, mean=4, sd=1.1),1)
after <- round(truncnorm::rtruncnorm(n=num_students*2, a=1, b=7, mean=2.5, sd=1.3),1)+c(rep(0, num_students),rep(0.8, num_students))

data <- tibble(
  学生No.=rep(1:num_students,4),
  #実験パターン=rep(rep(c("観葉植物→X印", "X印→観葉植物"), each=num_students/2),4),
  条件 = c(rep("実験群", num_students), rep("対照群", num_students), rep("実験群", num_students), rep("対照群", num_students)),
  時期 = c(rep("見る前", each = num_students*2), rep("見た後", each = num_students*2)),
  ストレス = c(before, after)
)
