...
このような2つ以上の説明変数を持った分散分析では，**`aov()`関数を行ったあとに，`car`パッケージの`Anova()`関数を実行する**必要がある．

Anova()関数の引数は以下の通りである．

- 第1引数: aov()関数の出力結果を収めたオブジェクト
- 第2引数 type: `type=2`とする(ほかにtype=3もある)．`type=2`は交互作用を考慮しない場合に用いる．（1と3があり，`type=3`は交互作用を考慮に入れる場合に用いる．type=1はaovと同じ結果となるだけなので用いることはほぼない．）
...
こうした交互作用を含んだ二要因分散分析を行う方法も，基本的に[一要因分散分析（対応あり）](#一要因分散分析（対応あり）)と大して変わらない．すなわち，使うコマンドは`aov()`関数と`car`パッケージの`Anova()`関数である．違う点は，`aov()`関数に与えるモデル式に交互作用項(**要因1:要因2**と表現する)が加わることと，`Anova()`関数の`type`引数に`3`を指定することである．
