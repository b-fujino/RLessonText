...
:::work
ある研究者が，オフィスデスクに置かれた観葉植物が，オフィスワーカーのストレスを軽減する効果があるのかを検討するために次の実験を行った．具体的には，被験者は20名の学生として，各被験者は，まず，オフィスでの事務作業に見立てた単純な作業（2桁×1桁の掛け算をひたすら計算し続ける課題）を15分間行ってもらった．その後，現在の心理的なストレス状態を測定するためのアンケートに回答してもらった後に，被験者を二つのグループに分けて，一方には1分間オフィスデスクの中央に置かれた観葉植物を見てもらい，もう一方にはデスクの中央に書かれたX印を見てもらった．デスク上には観葉植物ないぢは×印以外は何もない状態にしていた．1分間それらを見てもらった後に二ため心理的なストレス状態を測定するアンケートに回答してもらった．そのあと，15分間の休憩時間を置いたうえで，先ほどと同様の計算課題を行ってもらいストレスの測定も行ったあと，今度は先ほど観葉植物を見てもらったグループにはデスク上のX印を，先ほどX印を見てもらったグループには観葉植物を，それぞれ1分間見てもらい，その後ストレス状態を測定した．以上の実験を行った結果，次のデータが得られた．[このデータ](./practice/08_work_anova_
4.csv)を用いて，観葉植物がストレス軽減に効果があるのかを検討せよ．

:::ref
被験者内計画での実験を行う際には，各被験者は設定したすべての実験条件を体験することになる．その場合，どの順序で各条件を体験したのかが結果に影響を与えてしまう可能性がある（例えば，どういう条件であっても最初に体験した条件は印象が強くなる，など）．こうした順序効果の影響を排除するために，実施順序を入れ替えた群を設け，元の群のデータと実施順序を入れ替えた群のデータを合算する，という方法が一般によく取られる．被験者内計画での実験ではこの実施順序の影響を如何に除くかが非常に重要である．
