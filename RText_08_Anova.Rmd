...
# 二要因分散分析(対応なし)／繰り返しのある2元配置分散分析／被験者間計画
...
このようなデータの場合，理解度テストの「テスト時期」（受講前, 受講後）のみ対応ありであり，「講義方法」（従来型，新型）は対応なしである．
...
#### テスト時期を基軸とした単純主効果検定
今度はテスト時期を基軸として見る．この場合，受講前，受講後ごとの従来型，新型の差を見ることになる．この場合も従来型，新型という2つの群の比較なので，単純にT検定（対応あり）をすればよい．

```{r}
# 受講前の場合
t.test(data_old_before$理解度, data_new_before$理解度, paired=TRUE)

# 受講後の場合
t.test(data_old_after$理解度, data_new_after$理解度, paired=TRUE)
```
...
このように可視化すると，受講前の状態では講義の方式による差はみられず，受講後にはいずれの群も有意に理解度が向上していること，さらには新型の場合には講義後の理解度が従来型に比べて有意に高いことが一目瞭然で把握できる．
